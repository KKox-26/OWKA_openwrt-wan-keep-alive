...
download_files()
{
	...
	wget -q --show-progress --no-check-certificate --no-cache https://raw.githubusercontent.com/KKox-26/OWKA_openwrt-wan-keep-alive/master/restart-router.sh -O $DIR/restart-router.sh && chmod +x $DIR/restart-router.sh
	wget -q --show-progress --no-check-certificate --no-cache https://raw.githubusercontent.com/KKox-26/OWKA_openwrt-wan-keep-alive/master/wankeepalive -O /etc/init.d/wankeepalive && chmod +x /etc/init.d/wankeepalive
	echo "..."
